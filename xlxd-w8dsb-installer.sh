...
# This is N5AMD Script modified as many of the PHP links are no longer working
#W8DSB
#Lets begin-------------------------------------------------------------------------------------------------
WHO=$(whoami)
if [ "$WHO" != "root" ]
...
then
  echo ""
  echo "This script is only tested in ubuntu 18. "
  exit 0
fi
DIRDIR=$(pwd)
...
clear
echo ""
echo "XLX uses 3 digit numbers or three letters for its reflectors. For example: 032, 999, 099."
...
echo ""
echo "------------------------------------------------------------------------------"
echo " you will need to go to /etc/apache2/sites-enabled"
echo " edit the file ==> nano 000-default.conf"
echo " restart apache2 ==> systemctl restart apache2"
echo "------------------------------------------------------------------------------"
